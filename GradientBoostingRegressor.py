...
def apply_kmeans(df, n_clusters=3):
    ...
    return df, centroids
def apply_gbr_model(df, suitable_df):
    ...
    df_scaled = scaler.transform(df[common_features])
    df["Predicted_Label"] = model.predict(df_scaled)
    y_pred = model.predict(X_scaled)
    r2 = r2_score(y, y_pred)
    mae = mean_absolute_error(y, y_pred)
    print(f"Gradient Boosting Regression Performance: R² Score = {r2:.4f}, MAE = {mae:.4f}")
...
if __name__ == "__main__":
    file_path1 = r'C:\Users\naina\Downloads\Resume Project\ML Based Coral Water Analysis and Optimization\Regression Algorithm\Gradient Boosting Regressor\near_suitable_data.csv'
    ...
    file_path2 = r'C:\Users\naina\Downloads\Resume Project\ML Based Coral Water Analysis and Optimization\Regression Algorithm\Gradient Boosting Regressor\suitable.csv'
    suitable_df = pd.read_csv(file_path2)
    far_unsuitable_adjusted = apply_gbr_model(far_unsuitable_adjusted, suitable_df)
